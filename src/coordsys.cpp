...
Axis::Axis(widget_axis_data wd_in, axis_data ad_in) :
    wd{wd_in}, ad{ad_in}
{
...
std::vector<double> Axis::get_major_pos() const
{
    ...
    // sweep range is between ad.rng.min - ad.ticks.major_delta and ad.rng.max + ad.ticks.major_delta
    ...
    x{x_in}, y{y_in}, cd{cd_in}, title{cd.title.c_str()}
{
    ...
    // fmt::print("Coordsys::Coordsys: x.target_ratio = {:.3}\n", cd.x_rng_major_delta_target_ratio);
    // fmt::print("Coordsys::Coordsys: y.target_ratio = {:.3}\n\n", cd.y_rng_major_delta_target_ratio);
}
                              ...
                              double new_ymax)
{
    ...
    if (adx.rng.min != new_xmin || adx.rng.max != new_xmax)
    {
        ...
        adx.ticks.major_delta =
            get_new_delta(adx.rng.min, adx.rng.max, adx.ticks.major_delta, new_xmin, new_xmax);
    ...
    if (ady.rng.min != new_ymin || ady.rng.max != new_ymax)
    {
        // if there is some change
        // set new delta, as well as new min and new max
        ady.ticks.major_delta =
            get_new_delta(ady.rng.min, ady.rng.max, ady.ticks.major_delta, new_ymin, new_ymax);
                                    ...
                                    double ytarget_ratio)
{
    ...
    if (adx.rng.min != new_xmin || adx.rng.max != new_xmax)
    {
        ...
        adx.ticks.major_delta =
            get_new_delta_wheel_zoom(new_xmin, new_xmax,
                                     adx.ticks.major_delta, xtarget_ratio);
    ...
    if (ady.rng.min != new_ymin || ady.rng.max != new_ymax)
    {
        ...
        ady.ticks.major_delta =
            get_new_delta_wheel_zoom(new_ymin, new_ymax,
                                     ady.ticks.major_delta, ytarget_ratio);

        y = Axis(wdy, ady);
    }
}

double Coordsys::get_new_delta_wheel_zoom(double new_min, double new_max,
                                          double delta, double target_ratio)
{
    ...
    // fmt::print(
    //     "new_ratio = {:.3}, delta = {:.3}, target_ratio = {:.3}, new_delta = {:.3}\n\n",
